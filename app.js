...
function renderMap(){const svg=document.getElementById('map');svg.innerHTML='';const ns='http://www.w3.org/2000/svg';
  ...
  CITIES.forEach(c=>{const g=document.createElementNS(ns,'g');g.dataset.id=c.id;const node=document.createElementNS(ns,'circle');node.setAttribute('cx',c.x);node.setAttribute('cy',c.y);node.setAttribute('r',c.id===GAME.currentCity?8:6);node.setAttribute('class',c.id===GAME.currentCity?'player-node':'city-node');node.addEventListener('click',()=>{GAME.selectedCity=c.id;renderMap();renderAll();showCityMarket(c.id);});node.addEventListener('mouseenter',e=>showMapTooltip(e,c));node.addEventListener('mouseleave',()=>document.getElementById('mapTooltip').classList.add('hidden'));g.appendChild(node);const txt=document.createElementNS(ns,'text');txt.setAttribute('x',c.x+9);txt.setAttribute('y',c.y-9);txt.setAttribute('class','city-label');txt.textContent=c.name;g.appendChild(txt);svg.appendChild(g)});
}
function showCityMarket(cityId){const city=cityById(cityId);if(!city)return;document.getElementById('cityMarketTitle').textContent=`Mercato di ${city.name}`;document.getElementById('cityMarketSubtitle').textContent=`Giorno ${GAME.day} · ${SEASONS[seasonIndex(GAME.day)].name} · prezzi correnti in questa città.`;const body=document.getElementById('cityMarketBody');body.innerHTML='';GOODS.forEach(g=>{const q=localQuality(city,g);const base=marketPrice(city,g);const sellPrice=base*(0.92+q*0.012)*(0.98+GAME.employees.trader*0.015);const row=document.createElement('tr');row.innerHTML=`<td><strong>${g.name}</strong><br><small class="muted">Produzione: ${g.homeRegion}</small></td><td><span class="quality">${q}/10 <span class="quality-bar"><i style="width:${q*10}%"></i></span></span></td><td>${availableQty(city,g)}</td><td class="price">${fmt(base)}</td><td class="price">${fmt(sellPrice)}</td><td>${g.perishable?`${g.shelf} gg`:'No'}</td>`;body.appendChild(row)});document.getElementById('cityMarketModal').classList.remove('hidden')}
